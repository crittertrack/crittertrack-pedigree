...
const ANIMAL_TEXT_FIELDS = {
    ...
    sheddingRecords: 'shedding record notes',
    moltingRecords: 'molting record notes',
    waterParameterChecks: 'water quality check notes',
...
const getUsersAnimals = async (appUserId_backend, filters = {}) => {
    ...
    const slimFields = filters.slim === 'true' || filters.slim === true
        ? 'id_public creatorId creatorId_public originalCreatorId name prefix suffix species gender birthDate ' +
          ...
          'color coat earset markings eyeColor size carrierTraits geneticCode lifeStage ' +
          'ringId eartagNumber ' +
...
const updateAnimal = async (appUserId_backend, animalId_backend, updates) => {
    ...
    console.log('[updateAnimal] Updating with:', JSON.stringify({ 
        ...
        vaccinations: Array.isArray(updates.vaccinations) ? `${updates.vaccinations.length} records` : 'not an array or null',
        dewormingRecords: Array.isArray(updates.dewormingRecords) ? `${updates.dewormingRecords.length} records` : 'not an array or null',
