...
app.get('/api/users/breeder-directory', async (req, res) => {
    try {
        ...
        }).select('id_public personalName showPersonalName breederName showBreederName bio profileImage breedingStatus country state monthlyDonationActive lastDonationDate')
          .lean();
...
// Beta Survey Routes (Require authMiddleware)
const betaSurveyRoutes = require('./routes/betaSurveyRoutes');
app.use('/api/beta-survey', authMiddleware, betaSurveyRoutes);
